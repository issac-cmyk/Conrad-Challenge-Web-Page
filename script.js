...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initCarousel3() {
        ...
        // 创建包含所有组件的单张幻灯片，实现无缝循环
        const components = [
            { name: 'Arduino', image: 'Arduino.png' },
            { name: 'Buslinker', image: 'Buslinker.png' },
            { name: 'MPU6050', image: 'MPU6050.png' },
            { name: 'Pressure Sensor', image: 'Press.png' },
            { name: 'Servo Motor', image: 'Servo.png' }
        ];
        ...
        // 创建包含所有组件的连续布局
        const continuousLayout = document.createElement('div');
        continuousLayout.className = 'carousel-3-continuous';
        continuousLayout.style.cssText = `
            display: flex;
            width: max-content;
        `;
        
        // 添加组件到连续布局（添加两份，实现无缝循环）
        for (let i = 0; i < 2; i++) {
            components.forEach(component => {
                ...
                `;
                continuousLayout.appendChild(componentItem);
            });
        }
        
        carouselWrapper.appendChild(continuousLayout);
        
        let scrollPosition = 0;
        let scrollSpeed = 2.0; // 加快滚动速度
        ...
        function continuousScroll() {
            ...
            // 获取单个组件的宽度
            const componentWidth = continuousLayout.children[0].offsetWidth;
            const totalComponentsWidth = components.length * componentWidth;
            
            // 当滚动完一组组件后，无缝重置到开头
            if (scrollPosition >= totalComponentsWidth) {
                scrollPosition = 0;
            }
            
            // 应用滚动效果
            continuousLayout.style.transform = `translateX(-${scrollPosition}px)`;
